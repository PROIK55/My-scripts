...
if game.PlaceId == 9431156611 then
	...
	local Window = OrionLib:MakeWindow({Name = "Eclipse x  - Slap Royale!", HidePremium = true, IntroEnabled = false, SaveConfig = false, ConfigFolder = "OrionTest"})
	...
	Tab:AddLabel("Having issues? Contact byfronisuseless on Discord!")

	Tab:AddButton({
		Name = "IY (for mobile friendly use arceus x)",
		Callback = function()
				...
				if AntiRecord == true then
					for i, v in pairs(Words) do
						if v:lower():match("recording") or v:lower():match(" rec") or v:lower():match("record") or v:lower():match("discor") or v:lower():match(" disco") or v:lower():match(" disc") or v:lower():match("ticket") or v:lower():match("tickets") or v:lower():match(" ds") or v:lower():match(" dc") or v:lower():match("dizzy") or v:lower():match("dizzycord") or v:lower():match(" clip") or v:lower():match("proof") or v:lower():match("evidence") then
							AK:Set(false)
							game.Players.LocalPlayer:Kick("Someone probably threatened to record.".." ("..p.Name..")".." ("..message..")")
						end
					end
				end
			end)
	...
	GAI = Tab2:AddToggle({
		Name = "Get all items (only use after bus spawns)",
		Default = false,
		Callback = function(Value)
			GetAllItems = Value
			...
			elseif GetAllItems == true then
				OrionLib:MakeNotification({Name = "Error!",Content = "Match hasn't started yet. Try again later.",Image = "rbxassetid://7733658504",Time = 5})
				wait(0.05)
				GAI:Set(false)
			end
		end
	...
	Tab2:AddButton({
		Name = "Bomb the Bus (only use in the lobby)",
		Callback = function()
			repeat task.wait()
	...
	Tab2:AddButton({
		Name = "become a fucking gods hand user (needs 2 true power)",
		Callback = function()
			for i = 1, 2 do
	...
	Tab4:AddToggle({
		Name = "remove the annoying ass zone blindness",
		Default = false,
		Callback = function(Value)
			RemoveZone = Value
	...
	Tab4:AddButton({
		Name = "Free animation pack (type /e emotename)",
		Callback = function()
	...
	Tab5:AddSlider({
		Name = "choose how high you will jump",
		Min = 50,
		Max = 1000,
		Default = 50,
...
elseif game.PlaceId == 11520107397 then
	...
	local Window = OrionLib:MakeWindow({Name = "Eclipse X - Killstreak Only", HidePremium = true, IntroEnabled = false, SaveConfig = false, ConfigFolder = "OrionTest"})
	...
	Tab:AddLabel("Hello and welcome to BiU GUI!")
	...
	Tab:AddButton({
		Name = "thanos snap this entire GUI",
		Callback = function()
			OrionLib:Destroy()
		end    
	})
	...
	Tab4:AddButton({
		Name = "View Slap Battles TESTING (bypasses no access kick)",
		Callback = function()
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab4:AddButton({
		Name = "View something (SR Testing Place)",
		Callback = function()
			local teleportFunc = queueonteleport or queue_on_teleport
			if teleportFunc then
				teleportFunc([[
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab4:AddButton({
		Name = "INFINITE SLAPPERINOS FARM (copies a script, put it in autoexec)",
		Callback = function()
	...
	Tab2:AddToggle({
		Name = "Slap aura (69% risk of AC kick)",
		Default = false,
		Callback = function(Value)
			SlapAura = Value
			while SlapAura do
	...
	Tab2:AddButton({
		Name = "Breaks in KS Only (dont toggle anything)",
		Callback = function()
	...
	Tab2:AddTextbox({
		Name = "frame someone for exploiting (Rojo spam required)",
		Default = "Username",
		TextDisappear = false,
		Callback = function(Value)
	...
	Tab2:AddToggle({
		Name = "Null spam (works with anything 😎)",
		Default = false,
		Callback = function(Value)
			NullSpam = Value
			while NullSpam do
	...
	Tab2:AddToggle({
		Name = "Retro spam (works with anything 😎)",
		Default = false,
		Callback = function(Value)
			RetroSpam = Value
			while RetroSpam do
	...
	Tab2:AddToggle({
		Name = "enter arena automatically",
		Default = false,
		Callback = function(Value)
	...
	Tab4:AddToggle({
		Name = "Auto change nametag (NOT FE!)",
		Default = false,
		Callback = function(Value)
	...
	Tab4:AddDropdown({
		Name = "Rojo charge VFX, MIGHT FLING WHEN SPAMMED",
		Default = "",
	...
	Tab4:AddToggle({
		Name = "Spam zombie sound (works on anything)",
		Default = false,
		Callback = function(Value)
	...
	Tab4:AddToggle({
		Name = "Spam error sound (works on anything)",
		Default = false,
		Callback = function(Value)
...
game:GetService("TeleportService"):Teleport(6403373529)
	...
	Tab5:AddLabel("when in the elude maze I added a counter feature")

	Tab5:AddButton({
		Name = "Get the chain glove (needs 1K slaps)",
		Callback = function()
...
game:GetService("TeleportService"):Teleport(6403373529)
	...
	Tab5:AddButton({
		Name = "get orange lone the badge",
		Callback = function()
			...
			while AntiAdmins do
				for i,v in pairs(game.Players:GetChildren()) do
					if v:GetRankInGroup(9950771) >= 2 then
						AntiKick = false
						game.Players.LocalPlayer:Kick("Administrator detected! Username is:".." ("..v.Name..")")
						break
					end
				end
				task.wait()
	...
	AR = Tab3:AddToggle({
		Name = "anti ragdoll (⚠CHARACTER RESET WARNING)",
		Default = false,
		Callback = function(Value)
			AntiRagdoll = Value
			if AntiRagdoll then
...
elseif game.PlaceId == 11828384869 then
	...
	local Window = OrionLib:MakeWindow({Name = "Eclipse X | Elude Maze", HidePremium = true, IntroEnabled = false, SaveConfig = false, ConfigFolder = "OrionTest"})
	...
	Tab2:AddButton({
		Name = "View Slap Battles TESTING (loads different script)",
		Callback = function()
			local teleportFunc = queueonteleport or queue_on_teleport
			if teleportFunc then
				teleportFunc([[
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab2:AddButton({
		Name = "View something (sr test place)",
		Callback = function()
			local teleportFunc = queueonteleport or queue_on_teleport
			if teleportFunc then
				teleportFunc([[
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab2:AddButton({
		Name = "elude maze, slapple farm doesnt exist",
		Callback = function()
			setclipboard("loadstring(game:HttpGet('https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Farm'))()")
		end    
	})
	...
	Tab3:AddButton({
		Name = "Get your chain (Needs 1k slaps)",
		Callback = function()
			if game.Players.LocalPlayer.leaderstats.Slaps.Value >= 1000 then
				local teleportFunc = queueonteleport or queue_on_teleport
				if teleportFunc then
...
elseif game.PlaceId == 9020359053 or game.PlaceId == 9412268818 then
	...
	local Window = OrionLib:MakeWindow({Name = "Eclipse X (testing place)", HidePremium = true, IntroEnabled = false, SaveConfig = false, ConfigFolder = "OrionTest"})
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	local Window = OrionLib:MakeWindow({Name = "Eclipse X - Normal SB", HidePremium = true, IntroEnabled = false, SaveConfig = false, ConfigFolder = "OrionTest"})
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab4:AddButton({
		Name = "View something (sr testing server)",
		Callback = function()
			local teleportFunc = queueonteleport or queue_on_teleport
			if teleportFunc then
				teleportFunc([[
...
loadstring(game:HttpGet(("https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Battles")))()
	...
	Tab4:AddButton({
		Name = "FARM THE SLAPPERINOS (COPIES A SCRIPT, PUT IN SUTOEXEC)",
		Callback = function()
			setclipboard("loadstring(game:HttpGet('https://raw.githubusercontent.com/ionlyusegithubformcmods/1-Line-Scripts/main/Slap%20Farm'))()")
		end    
	})
	...
	Tab2:AddTextbox({
		Name = "accuse someone of exploiting (rojo spam required)",
		Default = "Username",
		TextDisappear = false,
		Callback = function(Value)
			if Value == "Me" or Value == "me" or Value == "Username" or Value == "" then
	...
	Tab2:AddButton({
		Name = "Cancel shadow realm imprisonment",
		Callback = function()
			Cancel = true
			wait(0.1)
			Cancel = false
	...
	Tab2:AddButton({
		Name = "Kick player (za hando needed) (bugged)",
		Callback = function()
	...
	Tab2:AddToggle({
		Name = "Auto enter arena of chaos",
		Default = false,
		Callback = function(Value)
			AutoEnterArena = Value
			while AutoEnterArena do
	...
	Tab4:AddButton({
		Name = "Give reaper 20 souls (use after reaper slaps you)",
		Callback = function()
			for i = 1, 20 do
	...
	RBSF = Tab5:AddToggle({
		Name = "Replica & baller slap farmo (get BALLER)",
		Default = false,
		Callback = function(Value)
			...
			elseif ReplicaBallerFarm == true then
				OrionLib:MakeNotification({Name = "Error",Content = "You don't have Baller equipped. LMAO",Image = "rbxassetid://7733658504",Time = 5})
				wait(0.05)
				RBSF:Set(false)
			end
		end
	...
	RNS = Tab5:AddToggle({
		Name = "Rhythm auto clicker (Equip Rhythm)",
		Default = false,
		Callback = function(Value)
	...
	Tab5:AddToggle({
		Name = "Rainbow Character (golden needed)",
		Default = false,
		Callback = function(Value)
			Rainbow = Value
			while Rainbow do
	...
	Tab4:AddButton({
		Name = "fuck every tycoon in the server",
		Callback = function()
			for i = 1, 110 do
				for i,v in pairs(workspace:GetDescendants()) do
